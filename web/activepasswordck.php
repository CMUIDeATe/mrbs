...
$sql=$dbo->prepare("SELECT name  FROM mrbs_users WHERE password=:ak and name=:name");
$sql->bindParam(':name',$name,PDO::PARAM_STR, 10);
$sql->bindParam(':password',$ak,PDO::PARAM_STR, 32);
$sql->execute();
$no=$sql->rowCount();
...
$count=$dbo->prepare("update mrbs_users set password='$password' where name='$name'");
$count->execute();
$no=$count->rowCount();
//echo " No of records = ".$no; 
if($no==1){
