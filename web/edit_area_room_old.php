...
if (!empty($room))
{
  ...
  if ( isset($change_room) && (FALSE != $valid_email) )
  {
	...
	{
	  $sql = "DELETE FROM $tbl_permissions WHERE room_id=$room";
	  if(sql_command($sql) < 0)
	    {
	  ...
	  for($i = 0; $i < sizeof($room_auth_users); $i++)
	    {
	      $sql = $sql . "($room, '$room_auth_users[$i]')" . (($i + 1) == sizeof($room_auth_users) ? "" : ",");
	    }
	  if(sql_command($sql) < 0)
	    {
	      fatal_error(0, get_vocab("update_room_failed") . sql_error() . " " . $sql);
...
<form class="form_general" action="edit_area_room.php" method="post">
  ...
  <legend><?php echo get_vocab("editroom") ?></legend>
    ...
    <div>
    <label for="room_auth_users"><?php echo get_vocab("room_auth_users") ?>:</label>
    <select id="room_auth_users" name="room_auth_users[]" multiple="multiple" size="10">
    <?php
    $sql2="SELECT u.ID, u.user_login, p.room_id 
					FROM mrbs_users AS u LEFT OUTER JOIN mrbs_permissions AS p ON u.user_login=p.user_name AND room_id=$room 
					WHERE u.level < 2 
